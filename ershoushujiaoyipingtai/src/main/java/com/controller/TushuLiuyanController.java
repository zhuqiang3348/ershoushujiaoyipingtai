...
public class TushuLiuyanController {
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        TushuLiuyanEntity tushuLiuyan = tushuLiuyanService.getById(id);
        if(tushuLiuyan !=null){
            TushuLiuyanView view = new TushuLiuyanView();
            BeanUtils.copyProperties( tushuLiuyan , view );

            TushuEntity tushu = tushuService.getById(tushuLiuyan.getTushuId());
            if(tushu != null){
            ...
            YonghuEntity yonghu = yonghuService.getById(tushuLiuyan.getYonghuId());
            if(yonghu != null){
    ...
    public R save(@RequestBody TushuLiuyanEntity tushuLiuyan, HttpServletRequest request){
        ...
        tushuLiuyanService.save(tushuLiuyan);
        return R.ok();
    }
    ...
    public R update(@RequestBody TushuLiuyanEntity tushuLiuyan, HttpServletRequest request){
        ...
        String role = String.valueOf(request.getSession().getAttribute("role"));
        Wrapper<TushuLiuyanEntity> queryWrapper = new QueryWrapper<TushuLiuyanEntity>()
                .eq("id",0);

        logger.info("sql语句:"+queryWrapper);
        TushuLiuyanEntity tushuLiuyanEntity = tushuLiuyanService.getOne(queryWrapper);
        tushuLiuyan.setUpdateTime(new Date());
        if(tushuLiuyanEntity==null){
            tushuLiuyanService.updateById(tushuLiuyan);
            return R.ok();
        }else {
    ...
    public R delete(@RequestBody Integer[] ids){
        ...
        tushuLiuyanService.removeByIds(Arrays.asList(ids));
        return R.ok();
    }
    ...
    public R save( String fileName){
        ...
        try {
            List<TushuLiuyanEntity> tushuLiuyanList = new ArrayList<>();
            ...
            if(lastIndexOf == -1){
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        tushuLiuyanService.saveBatch(tushuLiuyanList);
                        return R.ok();
                    }
                }
            }
